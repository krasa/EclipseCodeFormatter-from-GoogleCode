...
public class EclipseCodeStyleManager extends DelegatingCodeStyleManager {
	...
	public void reformatText(@NotNull PsiFile psiFile, final int startOffset,
		...
		try {
			...
			if (virtualFile != null
					&& !canReformatWithEclipse(virtualFile, project)) {
				...
				if (editor != null) {
					...
					// etc.
					// ctrl+shift+enter
					document.setText(codeFormatterFacade.format(text,
							fixedStartOffset, endOffset, LINE_SEPARATOR));
					...
					Document writeTo = fileDocumentManager
							.getDocument(virtualFile);
					fileDocumentManager.saveDocument(writeTo);
					writeTo.setText(codeFormatterFacade.format(
							ioFile(virtualFile), LINE_SEPARATOR));
	...
	private boolean shouldAppendBlankLine(String lastImportGroup,
		...
		return !lastImportGroup.equals(currentImportGroup)
				&& !settings.getJoinedGroupAsList().contains(
						JoinedGroup.from(lastImportGroup, currentImportGroup));
	}
